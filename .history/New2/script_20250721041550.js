...
inputElements.forEach((element, i) => {
    element.addEventListener("change", function() {
        // Если чекбокс включен и количество равно 0
        if(element.checked && (parseInt(priceElements[i].value) === 0)) {
            priceElements[i].value = 1; // Устанавливаем минимальное количество
        }
        else if(!element.checked){
            priceElements[i].value = 0;
        }
